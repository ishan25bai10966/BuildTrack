...
> **Database**: PostgreSQL 18 with JDBC  
...
- **Apache Maven**: Version 3.9+ installed and accessible via PATH.
- **PostgreSQL Database Server**: PostgreSQL 16+ running locally on port 5432.
...
Build and run the application via Maven:
```bash
mvn compile exec:java -Dexec.mainClass="com.buildtrack.Main"
```
...
Execute the comprehensive automated test suite with Maven:
```bash
mvn test
```
...
- **Status**: Production-Ready Console Release
